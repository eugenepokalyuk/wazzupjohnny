...
import { Contact, CONTACTS } from '@entities/contact';
import { KONAMI_KEYS } from '@features/konami-cheat';
...
export const InsertCoin: FC<Props> = ({ cheat, onCheat }) => {
  ...
  const handleCoin = (contact: Contact) => {
    showToast('🪙 ' + contact.label.toUpperCase() + ' — ' + t.contacts.newGame);
  };
  ...
  return (
    <section id="coin" className={`${classes.sec} ${classes.coin}`}>
      <div className={classes.wrap}>
        <div className={classes.coinHead}>{t.contacts.banner}</div>
        <p className={classes.coinSub}>{t.contacts.subtitle}</p>
          ...
          {CONTACTS.map((contact) => (
            ...
            >
              ...
              <span className={classes.ct}>
                <b>{contact.label}</b>
                <span>{t.contacts[contact.id]}</span>
              </span>
            </a>
          ))}
        </div>

        <div className={classes.footerNote}>{t.contacts.thanks}</div>

        <div className={classes.secret}>
          <div className={classes.secretHint}>{t.contacts.secretHint}</div>
