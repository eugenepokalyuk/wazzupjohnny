...
export const AboutMock: AboutItem[] = [
  {
    ...
    description:
      'Я Frontend-разработчик с более чем трехлетним опытом создания интерактивных и удобных пользовательских интерфейсов. Моя специализация — React и TypeScript, а для управления состоянием я использую Redux и Redux-Saga, а также изучаю MobX. Мне знакомы современные инструменты разработки, включая сборщики Webpack и Vite, что позволяет мне создавать быстрые и надежные приложения',
  ...
  },
  {
    ...
    description:
      'Увлекаюсь сноубордингом и скейтбордингом, наслаждаясь скоростью и адреналином. Еще одна моя страсть — тяжелая музыка. Mick Gordon, Powerglove, Rammstein, Warmen и другие инструментальные треки сопровождают меня каждый день, заряжая энергией и вдохновением',
