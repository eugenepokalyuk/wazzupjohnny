...
const loginEpic = (action$) =>
  action$.pipe(
    ...
    },
    {
      ...
      listItems: [
        'Код становится чище и легче читается, так как логика API-запросов вынесена и не смешивается с потоком событий',
    ...
    },
    {
      type: ContentType.Paragraph,
      content:
        'Отличный выбор, если хочешь использовать мощь RxJS для управления асинхронностью в Redux. Правильное использование ofType, экшен-креаторов и отделение API-запросов делают код лаконичным, понятным и легко расширяемым',
    },
  ],
  externalLinks: [
    {
